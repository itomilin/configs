...
brPath            = "/sys/class/backlight/intel_backlight/brightness"
altMask           = mod1Mask
...
getVolume       = localScriptsPath ++ "volume_control.sh"
showCurrentTime = localScriptsPath ++ "show_current_time.sh"
...
keyBinds =
    [ (( 0, 0x1008ff13 ), spawn $ getVolume ++ " UP"                          )
    , (( 0, 0x1008ff11 ), spawn $ getVolume ++ " DOWN"                        )
    , (( 0, 0x1008ff12 ), spawn $ getVolume ++ " TOGGLE"                      )
    , (( 0, 0x1008ffb2 ), spawn $ "/usr/bin/amixer set Capture toggle"        )
    ...
    , (( altMask .|. shiftMask, xK_l ), spawn "/usr/bin/dm-tool lock"         )
    , (( 0, 0x1008ff03 ), spawn $ "current_br=$((`cat " ++ brPath ++ "`-10));\
                                   \ echo $current_br > " ++ brPath           )
    , (( 0, 0x1008ff02 ), spawn $ "current_br=$((`cat " ++ brPath ++ "`+10));\
                                   \ echo $current_br > " ++ brPath           )
    , (( altMask .|. shiftMask, 0x74 ), spawn $ showCurrentTime               )
    , (( mod4Mask, xK_p ), spawn $ "dmenu_run -fn 'xft:DejaVu Sans Mono:size=20:bold:antialias=true'"   )
    ]
...
--    spawnOnce "telegram"
    ...
    setWMName "LG3D" -- continue setWMname package FIX JAVA GUI APPS
